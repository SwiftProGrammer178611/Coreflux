...
export function initGame() {
    ...
    spawnEnemyCamps();
    const campWatchId = setInterval(campWatch, 300);
    ...
    whenEnterMap([{ x: 900, y: -200 }, { x: 900, y: 340 }]);
    return () => {
        // clear all intervals
        clearInterval(waveTime);
        clearInterval(campWatchId);
...
*/
let zones = [
    //{ x1: 140, y1: 100, x2: 300, y2: 150 },
    { type: "diamond", centerDiamond: 288, cy: 326, diamondHalfWidth: 120, diamondHalfHeight: 60 },
    { type: "diamond", centerDiamond: 349, cy: 412, diamondHalfWidth: 120, diamondHalfHeight: 60 },
    { type: "diamond", centerDiamond: 853, cy: 575, diamondHalfWidth: 30, diamondHalfHeight: 25 },
    { type: "diamond", centerDiamond: 851, cy: 621, diamondHalfWidth: 30, diamondHalfHeight: 25 },
    { type: "diamond", centerDiamond: 900, cy: 596, diamondHalfWidth: 30, diamondHalfHeight: 25 },
    { type: "diamond", centerDiamond: 940, cy: 626, diamondHalfWidth: 30, diamondHalfHeight: 25 },
    { type: "diamond", centerDiamond: 498, cy: 492, diamondHalfWidth: 30, diamondHalfHeight: 25 },
    { type: "diamond", centerDiamond: 100, cy: 789, diamondHalfWidth: 30, diamondHalfHeight: 30 },
];
...
//getComputedStyle is returns
//getPropertyValue does
...
const enemyImgW = 1560;
const enemyImgH = 840;
const enemy1FrmW = 120;
const enemy1FrmH = 120;

const enemystates = {
    idle: { row: 0, frames: 10 },
    walk: { row: 1, frames: 11 },
    death: { row: 2, frames: 9 },
};

function enemySpawn(path = generateRandomPath()) {
    const game = document.querySelector(".map");
    const enWrap = document.createElement("div");
    enWrap.className = "enemy";
    enWrap.style.position = "absolute";
    enWrap.style.transform = "scale(0.4)";
    //whats enWrap?

    const enemy = document.createElement("div");
    enemy.className = "enemy_spritesheet";
    enWrap.appendChild(enemy);
    const startPosX = path[0].x;
    const startPosY = path[0].y;
    game.appendChild(enWrap);
    enWrap.style.left = startPosX + "px";
    enWrap.style.top = startPosY + "px";
    ...
    startEnemyAnim(enemyDef);
    return enemyDef;

}

function startEnemyAnim(enemyDef) {
    enemyDef.walkId = setInterval(() => {
        /*so tis been 60 mins and 55 mins, close to 2 hrs */
        const pixelSize = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--pixel-size'));
        const s = enemystates[enemyDef.state];
        enemyDef.sprite.style.backgroundSize = `${enemyImgW * pixelSize}px ${enemyImgH * pixelSize}px`;
        enemyDef.sprite.style.backgroundPosition = `-${enemyDef.frame * enemy1FrmW * pixelSize}px -${s.row * enemy1FrmH * pixelSize}px`;
        enemyDef.frame = (enemyDef.frame + 1) % s.frames;
    }, 150)
}
...
function enemyFall(enemy) {
    ...
    enemy.state = "death"
    enemy.frame = 0;
    enemy.walkId = setInterval(() => {
        const pixelSize = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--pixel-size'));
        const s = enemystates[enemy.state];
        enemy.sprite.style.backgroundSize = `${enemyImgW * pixelSize}px ${enemyImgH * pixelSize}px`;
        enemy.sprite.style.backgroundPosition = `-${enemy.frame * enemy1FrmW * pixelSize}px -${s.row * enemy1FrmH * pixelSize}`;
        enemy.frame++;
        if (enemy.frame >= s.frames) {
            clearInterval(enemy.walkId);
            enemy.wrap.remove();
        }
    }, 150);
...
function whenEnterMap(path) {
    const game = document.querySelector(".map");

    const rocketEnter = document.getElementById("rocketEnter");
    const enWrap = document.createElement("div");
    enWrap.className = "mechEnter";
    enWrap.style.position = "absolute";
    enWrap.style.transform = "scale(0.4)";

    const mech1Rocket = document.createElement("div");
    mech1Rocket.className = "mech1Rocket_SpriteSheet";
    enWrap.appendChild(mech1Rocket);
    const startPosX = path[0].x;
    const startPosY = path[0].y;
...
function startRocketAnim(rocketDef) {
    rocketDef.walkId = setInterval(() => {
        const pixelSize = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--pixel-size'));
        const s = rocketstates[rocketDef.state];
        rocketDef.sprite.style.backgroundSize = `${rocketImgW * pixelSize}px ${rocketImgH * pixelSize}px`;
